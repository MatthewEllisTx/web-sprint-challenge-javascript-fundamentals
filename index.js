...
// when nestedFunction() is called it does not have a local variable internal, and so it looks in the scope above it and sees that myFunction does
// if we returned nestedFunction then it would still have access to internal even after myFunction is terminated
...
function animalNames(data){
  return data.map(description => `name: ${description.animal_name}, scientific: ${description.scientific_name}`);
}

//I just noticed I was supposed to use forEach for this so here you go
// function animalNames(data){
//   const names = [];
//   data.forEach(description => names.push(`name: ${description.animal_name}, scientific: ${description.scientific_name}`));
//   return names;
// }
// I tested it with the tester and it works
...
function lowerCaseNames(data){
  return data.map(description => description.animal_name.toLowerCase());
}
...
function lowPopulationAnimals(data){
  return data.filter(description => description.population < 5);
}
...
function USApop(data){
  return data.reduce((accumulator, currentAnimal) => accumulator + currentAnimal.population, 0);
}
